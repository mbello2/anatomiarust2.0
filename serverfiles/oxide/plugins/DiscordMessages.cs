...
{
    [Info("DiscordMessages", "Slut", "1.8.4", ResourceId = 2486)]
    class DiscordMessages : CovalencePlugin
    {
        ...
        {
            FancyMessage message = new FancyMessage(content, false, new FancyMessage.Embeds[1] { new FancyMessage.Embeds(embedName, embedColor == 0 ? 3329330 : embedColor, JsonConvert.DeserializeObject<List<Fields>>(json)) });
            ...
            webrequest.Enqueue(url, payload, (code, response) =>
                {
                    if (code != 200 && code != 204)
                    {
                        if (response != null)
                        {
                            ...
                            {
                                PrintWarning($"Discord rejected that payload! Responded with \"{response}\" Code: {code}");
                            }
                        }
                        else
                        {
