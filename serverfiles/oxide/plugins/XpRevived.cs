...
{
    [Info("XpRevived", "Mattparks", "0.2.6", ResourceId = 2753)]
    ...
    class XpRevived : RustPlugin
    {
        ...
        {
            if (configs.options.lootRemoveScrap && entity is LootContainer)
            {
            ...
            if (args.Length == 1)
            {
                ...
                foreach (ItemData item in configs.options.itemDatas)
                {
                    if (item.unlockLevel == level)
                    {
                        ...
                        message.Append($"\n<color={(learned ? configs.display.textColourLearned : configs.display.textColourNotLearned)}> - {GetItemDefinition(item.shortName).displayName.english} ({item.costXP} XP)</color>");
                    }
                }
        ...
        {
			if (configs.options.unlockAllOnLevel)
			{
				return;
			}
        ...
        private void ConsoleResetBlueprints(ConsoleSystem.Arg arg)
        {
            ...
            foreach (var val in storedData.playerData)
            {
        ...
        public void IncreaseXp(BasePlayer player, float amount, bool updateAFK = true)
        {
            ...
            if (!configs.options.unlockAllOnLevel && Math.Floor(oldXP) != Math.Floor(playerData.xp))
            {
                player.SendConsoleCommand("gametip.hidegametip");
        ...
        {
			if (!configs.display.hudEnabled)
			{
				return;
			}
			
            string colourProgressLevel = UI.Color(configs.display.hudColourLevel, 1.0f);
            string colourProgressXp = UI.Color(configs.display.hudColourXp, 1.0f);
            PlayerData playerData = GetPlayerData(player.userID);
            ...
            if (refreshAll)
            {
                ...
                UI.LoadImage(ref elementPanel, uiPanel, configs.display.levelIcon, "0.025 0.65", "0.092 0.895");
				
				if (!configs.options.unlockAllOnLevel)
				{
					UI.LoadImage(ref elementPanel, uiPanel, configs.display.xpIcon, "0.025 0.10", "0.092 0.365");
				}
            ...
            var elementLevel = UI.CreateElementContainer(uiLevel, colourBackground,
                ...
                (0.122f + configs.display.offsetX) + " " + (0.1 + configs.display.offsetY));
