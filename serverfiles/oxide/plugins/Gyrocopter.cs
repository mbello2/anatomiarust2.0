...
{
    [Info("Gyrocopter", "ColonBlow", "1.2.2", ResourceId = 2521)]
    class Gyrocopter : RustPlugin
    {

        //Fixed issue where copter would try a "landing" when taking off causing copter now to move.
        //Removed gyrocopter.fly permssions.. players will have to lock them down to prevent others from using them.
        ...
        private object CanDismountEntity(BaseMountable mountable, BasePlayer player)
        {
            ...
            {
                if (mountable.GetComponent<BaseEntity>() != activecopter.entity) return null;
