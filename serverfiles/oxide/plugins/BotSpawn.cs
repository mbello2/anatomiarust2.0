...
{
    [Info("BotSpawn", "Steenamaroo", "1.7.3", ResourceId = 2580)] 
                    ...
                    if (active != null && active.ToString().Contains("flashlight")) 
                    {
                        foreach (Item item in npcPlayer.inventory.containerBelt.itemList)                                                                   //pick one at random to start with
                        {
                            ...
                            if (item.position == weapons[index])
                            {
                                if (heldEntity1 != null)
                                    heldEntity1.SetHeld(false);
                                UID = npcPlayer.inventory.containerBelt.GetSlot(weapons[index]).uid;
        ...
        object OnNpcPlayerTarget(NPCPlayerApex npcPlayer, BaseEntity entity)
        {
            ...
            if (entity is BasePlayer) 
            {
                ...
                if (!victim.userID.IsSteamId() && !(victim is NPCPlayer) && conf.Ignore_HumanNPC)                                                                           //stops bots targeting humannpc
                    return true;
                npcPlayer.Hurt(new HitInfo() { Initiator = victim });                                                                                                       //murderers fire in wrong direction, until they have been hurt?!
            }
        ...
        public class botData : MonoBehaviour
        {
            ...
            private void OnCollisionEnter(Collision collision)
            {
                ...
                if (inAir)
                {
                    ...
                    NavMeshHit hit;
                    if (NavMesh.SamplePosition(botapex.transform.position, out hit, 100f, NavMesh.AllAreas)) 
                    {
                        var navDistance = botapex.transform.position.y - hit.position.y;
                        if (navDistance < 2 && navDistance > 0)
                            closeToNav = true; 
                    }
                    if (collision.collider.name.Contains("Terrain") || closeToNav)
                    {
                        rb.isKinematic = true; 
                        rb.useGravity = false;
                        botapex.gameObject.layer = 17; 
